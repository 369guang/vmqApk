        ...
        newThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                while (true) {
                    ...
                    heart = read.getString("heart", "");
                    key = read.getString("key", "");

                    if (heart != "" && heart != null) {
                        //这里写入子线程需要做的工作
                        String url = NeNotificationService2.heartURL(heart, key);
...
//                Log.d(TAG, "key:" + key);
                ...
                if (pkg.equals("com.eg.android.AlipayGphone")) {
                    ...
                    if (content.equals("这是一条测试推送信息，如果程序正常，则会提示监听权限正常")) {
                        ...
                        handlerThree.post(new Runnable() {
                            public void run() {
                                Toast.makeText(getApplicationContext(), "监听正常，如无法正常回调请联系平台反馈！", Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
                }
...
//        String sign = md5(type + "" + price + t + key);
    ...
    public static String decode64(String string) {
        ...
        try {
            decodeKey = new String(Base64.getDecoder().decode(string), "UTF-8");
            String[] keyTmp = decodeKey.split("\\.");
            return keyTmp[3];
    ...
    public static String heartURL(String url, String key) {
        String timestamp = String.valueOf(new Date().getTime());
        String t = timestamp.substring(0, 10);
        String sign = md5(t + key);
        String result = url + "?t=" + t + "&sign=" + sign;
        return result;
    }
}
